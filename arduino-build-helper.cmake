...
set(ARDUINO_VER "10819" CACHE STRING "Arduino version define, 10819 == 1.8.19")
...
function(add_arduino_library LIBNAME LIBBASEDIR)
	...
	if(IS_DIRECTORY "${LIBBASEDIR}/${LIBNAME}")
		...
		set(LIBDIRUTIL "${LIBDIR}/utility")
		set(LIBFORMAT "1.0")
		# probe for library format
		if(IS_DIRECTORY "${LIBDIR}/SRC")
			set(LIBDIR "${LIBDIR}/SRC")
			set(LIBFORMAT "1.5")
		elseif(IS_DIRECTORY "${LIBDIR}/Src")
			set(LIBDIR "${LIBDIR}/Src")
			set(LIBFORMAT "1.5")
		elseif(IS_DIRECTORY "${LIBDIR}/src")
			set(LIBDIR "${LIBDIR}/src")
			set(LIBFORMAT "1.5")
		elseif(IS_DIRECTORY "${LIBDIR}/UTILITY")
			set(LIBDIRUTIL "${LIBDIR}/UTILITY")
		elseif(IS_DIRECTORY "${LIBDIR}/Utility")
			set(LIBDIRUTIL "${LIBDIR}/Utility")
		elseif(IS_DIRECTORY "${LIBDIR}/utility")
		...
		#add sources according to library directory format: https://arduino.github.io/arduino-cli/latest/library-specification
		if(${LIBFORMAT} STREQUAL "1.0")
			file(GLOB LIBSOURCES "${LIBDIR}/*.cpp" "${LIBDIR}/*.c" "${LIBDIR}/*.S" "${LIBDIRUTIL}/*.cpp" "${LIBDIRUTIL}/*.c" "${LIBDIRUTIL}/*.S")
		elseif(${LIBFORMAT} STREQUAL "1.5")
			file(GLOB LIBSOURCES "${LIBDIR}/*.cpp" "${LIBDIR}/*.c" "${LIBDIR}/*.S")
		else()
			message(FATAL_ERROR "Failed to detect valid library format!")
		endif()
		...
		#finalize
		if(LIBADDED)
			message(STATUS "Added ${LIBFORMAT}-format library \"${LIBNAME}\" from ${LIBDIR}")
...
function(add_arduino_sketch SKETCHNAME SKETCHDIR)
	...
	if((EXISTS ${SKETCHDIR}/${SKETCHNAME}) AND (IS_DIRECTORY ${SKETCHDIR}/${SKETCHNAME}))
		if(NOT TARGET ${SKETCHNAME})
			...
			file(GLOB SKETCHHEADERS ${SKETCHDIR}/${SKETCHNAME}/*.h ${SKETCHDIR}/${SKETCHNAME}/*.hpp ${SKETCHDIR}/${SKETCHNAME}/*.hxx)
			add_executable(${SKETCHNAME} ${SKETCHSOURCES} ${SKETCHHEADERS})
...
target_compile_options(arduino-core PRIVATE "-Wno-unused-parameter")
list(APPEND ARDUINO_ADDED_LIBS arduino-core)
