...
function(add_arduino_sketch SKETCHDIR)
    file(TO_CMAKE_PATH "${SKETCHDIR}" SKETCHDIR)
    if((EXISTS ${SKETCHDIR}) AND (IS_DIRECTORY ${SKETCHDIR}))
        get_filename_component(SKETCHNAME ${SKETCHDIR} NAME)
        if(NOT TARGET ${SKETCHNAME})
            file(GLOB SKETCHSOURCES ${SKETCHDIR}/*.ino ${SKETCHDIR}/*.cpp ${SKETCHDIR}/*.S ${SKETCHDIR}/*.c)
            if(SKETCHSOURCES)
                add_executable(${SKETCHNAME} ${SKETCHSOURCES})
                set_target_properties(${SKETCHNAME} PROPERTIES OUTPUT_NAME "${SKETCHNAME}.elf")
                set_directory_properties(PROPERTIES ADDITIONAL_MAKE_CLEAN_FILES "${SKETCHNAME}.hex;${SKETCHNAME}.eep;${SKETCHNAME}.lst")
                target_link_libraries(${SKETCHNAME} arduino-core)
                foreach(EXTRA_DEP IN LISTS ARGN)
                    target_link_libraries(${SKETCHNAME} ${EXTRA_DEP})
                endforeach()
            else()
                message(FATAL_ERROR "No source files found in sketch directory ${SKETCHDIR}")
            endif()
        endif()
    else()
        message(FATAL_ERROR "Sketch directory is missing ${SKETCHDIR}")
    endif()
endfunction()
